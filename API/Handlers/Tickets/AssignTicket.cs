    ...
    public class AssignTicket
    {
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                //If dev_id is null, set ticket's dev to null, save and exit.
                if (request.dev_id == null)
                {
                    ticket.developer = null;
                    var dev_null_success = await context.SaveChangesAsync() > 0;
                    if (dev_null_success) return Unit.Value;
